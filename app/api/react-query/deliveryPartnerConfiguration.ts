...
export type UpdateGeneralSettingsRequest = Omit<GeneralSettings, "id" | "updatedAt">;

export type UpdateNotificationSettingsRequest = Omit<NotificationSettings, "id" | "updatedAt">;
...
const fetchGeneralSettings = async () => {
    const res = await deliveryApiClient.get(`/delivery-partner/general-configuration`);
    return res.data.data as GeneralSettings;
};

const updateGeneralSettings = async (settings: UpdateGeneralSettingsRequest): Promise<GeneralSettings> => {
...
const fetchQuestions = async (): Promise<QuestionsResponse> => {
    const response = await deliveryApiClient.get(`/delivery-partner/drivers-question`);
...
const createQuestion = async (questionData: CreateQuestionRequest) => {
    const user = await JSON.parse(localStorage.getItem("deliveryPartnerUser") || "{}");
    return deliveryApiClient.post(`/delivery-partner/drivers-question`, { ...questionData, partnerId: user.id });
};

const updateQuestion = async (questionData: UpdateQuestionRequest): Promise<Question> => {
    const user = await JSON.parse(localStorage.getItem("deliveryPartnerUser") || "{}");
    return deliveryApiClient.put(`/delivery-partner/drivers-question/${questionData.id}`, {
        ...questionData,
        partnerId: user.id
    });
};
...
const deleteQuestion = async (questionId: string): Promise<void> => {
    await deliveryApiClient.delete(`/delivery-partner/drivers-question/${questionId}`);
};
...
const fetchOnboardRequirements = async (): Promise<OnboardRequirementsResponse> => {
    const response: any = await deliveryApiClient.get(`/delivery-partner/requirements`);
    return Promise.resolve({
        onboardRequirements: response?.data?.onboardRequirements || ONBOARD_REQUIREMENT_DEFINITIONS,
        vehicleCheckFields: response?.data?.vehicleCheckFields || VEHICLE_CHECK_FIELD_DEFINITIONS
    });
};

const updateOnboardRequirements = async (data: any) => {
...
const fetchNotificationSettings = async (): Promise<NotificationSettings> => {
    ...
    console.log("debug:Notification Settings:", res.data);
    return {
...
export const useUpdateGeneralSettings = () => {
    ...
    return useMutation({
        mutationFn: ({ data }: { data: UpdateGeneralSettingsRequest }) => updateGeneralSettings(data),
        onSuccess: () => {
