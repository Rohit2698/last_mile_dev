...
import {
  useDispensaryConnectionCode,
  useRegenerateDispensaryCode,
  useDispensaryConnections,
  useDispensaryConnectionRequests,
  useConnectToPartner,
  useApproveDispensaryRequest,
  useRejectDispensaryRequest,
  useDisconnectFromPartner,
} from "@/app/api/react-query/connections"
import { toast } from "react-toastify"
import { Check, Copy, RefreshCw, Truck, UserCheck, UserX, X } from "lucide-react"
...
export default function DispensaryConnectionsPage() {
  const [codeInput, setCodeInput] = useState("")

  const { data: codeData, isLoading: codeLoading } = useDispensaryConnectionCode()
  const { data: connections, isLoading: connectionsLoading } = useDispensaryConnections()
  const { data: requests, isLoading: requestsLoading } = useDispensaryConnectionRequests()

  const regenerateMutation = useRegenerateDispensaryCode()
  const connectMutation = useConnectToPartner()
  const approveMutation = useApproveDispensaryRequest()
  const rejectMutation = useRejectDispensaryRequest()
  const disconnectMutation = useDisconnectFromPartner()
  ...
  const handleConnect = () => {
    ...
    connectMutation.mutate(codeInput, {
      onSuccess: () => {
        toast.success("Connection request sent! Awaiting delivery partner approval.")
        setCodeInput("")
      },
      onError: (err: any) => {
      ...
      <div className="space-y-6 p-4 max-w-3xl">
        <div>
          ...
          <p className="text-muted-foreground">
            Connect with delivery partners using 6-digit codes
          </p>
        </div>
        ...
        <Card className="p-6 space-y-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Share this code with a delivery partner so they can connect with you.
            </p>
          </div>
          {codeLoading ? (
        ...
        <Card className="p-6 space-y-3">
          <div>
            <h3 className="text-lg font-semibold">Connect to a Delivery Partner</h3>
            <p className="text-sm text-muted-foreground">
              Enter the delivery partner&apos;s 6-digit code to send a connection request.
            </p>
          </div>
        ...
        {/* Pending Requests (incoming from delivery partners) */}
        <Card className="p-6 space-y-4">
          <div>
            <h3 className="text-lg font-semibold">Incoming Requests</h3>
            <p className="text-sm text-muted-foreground">
              Delivery partners requesting to connect with you.
            </p>
          </div>
          {requestsLoading ? (
              ...
              {requests.map((req) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">{req.deliveryPartner?.companyName}</p>
                      <p className="text-xs text-muted-foreground">{req.deliveryPartner?.email}</p>
                    </div>
                  </div>
        ...
        <Card className="p-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Delivery partners you are currently working with.
            </p>
          </div>
          {connectionsLoading ? (
            <p className="text-sm text-muted-foreground">Loading…</p>
              ...
              {connections.map((link) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-medium">{link.deliveryPartner?.companyName}</p>
                        {link.deliveryPartner?.isVerified && (
                          <Badge className="bg-green-500 text-white text-xs">Verified</Badge>
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground">{link.deliveryPartner?.email}</p>
                      {link.deliveryPartner?.phone && (
                        <p className="text-xs text-muted-foreground">{link.deliveryPartner.phone}</p>
                      )}
                    </div>
                  </div>
                  <Button
